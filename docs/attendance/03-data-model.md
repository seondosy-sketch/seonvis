...
Project List에는 이런 이력 테이블이 없다(`01-current-analysis.md` §6 근거) — 신규 생성 후 **출퇴근부는 이 테이블을 연계해서 읽기만** 한다(중복 관리 금지, 마스터 프롬프트 지시).
...
`lib/menuConfig.ts`의 `RESTRICTABLE_MENU_ITEMS`에 `{ key: 'attendance', label: '기술인 출퇴근부' }` 1건 추가(코드 변경 — DB 마이그레이션 아님, `06-implementation-plan.md`에서 다룸).
