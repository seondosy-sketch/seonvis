...
//
// 주간(개찰/진행중/발주예상)·월간(프로젝트) 모두 고정 행 수 제한이 없다 — app/api/hwpx/route.ts가
// 실제 데이터 수에 맞춰 표 행을 동적으로 늘리거나 줄이고, 대신 각자의 page budget 계산기
// (lib/hwpx/pageBudget.ts, lib/hwpx/monthlyPageBudget.ts)의 "자동 문서 높이 예산" 검사로 과도한
// 입력을 막는다. 여기 남은 validateWeeklyCapacity는 행 수 제한이 아니라 "수행 프로젝트 status
// 값이 개찰/진행중이 아닌 경우"만 검증한다 — status가 이상한 값이면 기존 코드는 개찰·진행중
// 어느 쪽에도 안 걸려서 조용히 문서에서 빠져버리기 때문이다. 월간에는 이런 status 구분이
// 없어 대응하는 검증 함수가 없다.
